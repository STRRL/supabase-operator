...
├── crds/                              # CRDs generated by `make manifests`
├── templates/                         # Deployment, RBAC, webhook, services
└── values.yaml                        # Chart values (metrics/webhook/etc.)

Samples are documented inline in quickstart; no repository samples directory.
```
